...
async function refreshTokens(): Promise<Tokens> {
  ...
  const res = await fetch(`${httpClient}/users/token/refresh/`, {
    method: "POST",
...
export async function apiFetch<T>(
  ...
  let res = await fetch(`${httpClient}${path}`, { ...init, headers });
  ...
  if (res.status === 401 && auth) {
    ...
    res = await fetch(`${httpClient}${path}`, { ...init, headers: headers2 });
  }
